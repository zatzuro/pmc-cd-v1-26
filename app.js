(function(){
  ...
  function openDetail(e){state.selected=e;const color=colorFor(e.Tipo);$('#detailContent').innerHTML=`<div style="--event-color:${color}"><div class="detail-type">${esc(e.Tipo)}</div><h2 class="detail-title" id="detailTitle">${esc(e.Nombre)}</h2>${e.Comercializable==='Sí'?`<span class="status-pill ${e.Disponibilidad==='Cupo lleno'?'full':''}">${esc(e.Disponibilidad)}</span>`:''}<div class="detail-meta"><div class="detail-row"><span>▣</span><div>${esc(formatRange(e))}</div></div><div class="detail-row"><span>◉</span><div>${e.Comercializable==='Sí'?'Comercializable':'No comercializable'}</div></div>${e.Enlace?`<div class="detail-row"><span>↗</span><a class="detail-link" href="${esc(e.Enlace)}" target="_blank" rel="noopener noreferrer">Ver enlace</a></div>`:''}</div>${e.Descripcion?`<div class="detail-description">${esc(e.Descripcion)}</div>`:''}<div class="detail-actions"><button class="button secondary" data-action="edit-selected">Editar</button><button class="button danger" data-action="delete-selected">Eliminar</button></div></div>`;$('#detailModal').hidden=false;lockBody(true)}
